...
Description: Display Thumbnail Grid using Featured Images. Warning: This upgrade removes support for bookmarks
Version: 6.1
Author: A. R. Jones
...
function thumbnailgrid_handler($atts) {
    ...
    $tg = new thumbnailgrid($atts);
    return $tg->thumbnailgrid_function();
    
}


add_action( 'wp_enqueue_scripts',  array('thumbnailgrid' , 'scripts_method' ));
...
class thumbnailgrid
{
    ...
    public function __construct($atts = NULL)
    {
       ...
       //If option to load css is set to footer, load it now.
        if ($css_load == "footer")
            wp_enqueue_style('sfly-tbgrdr-css');
        if (isset($atts))
        {
            $this->theatts = $atts;
    ...
    function scripts_footer()
    {
       ...
       'auto';
       wp_register_script('sf-impact-thumbnail-grid-script', plugins_url('js/thumbnailgrid.js', __FILE__), array('jquery'), '', true);
    ...
    public static function scripts_method()
    {
            ...
            $css_load = (get_option('sfly_tbgrid_load_styles', 'header')); //get the options
            wp_register_style( 'sfly-tbgrdr-css', plugins_url( 'css/' . $css_filename , __FILE__ ) );
    ...
    function defaults($atts)
    {
       ...
       $atts= shortcode_atts( array(
            ...
            'target' => NULL,               //Target
            'inclusive' => FALSE,           //Used with the before and after dates to make those dates inclusive instead of exclusive
            'has_thumbnails' => FALSE,
            'ignore_sticky_posts' => FALSE
                ), $atts  );
             if ($oldatts)  //Since this shortcode can take most Wp_Query params
                $atts = array_merge($atts, $oldatts); //merge in anything back in that is not in default array
             return $atts;
    }
    ...
    function getSettings(&$atts)
    {
            ...
            //unset all attributes that are not query attributes   
            unset($atts["height"]);
            unset($atts["width"]);
          ...
          //  this should never happen, left in because of old bug / failsafe
            ...
            $bHas_thumb = $this->boolVal($has_thumbnails);
            $bInclusive = $this->boolVal($inclusive);
           //Create the attributes for date range queries
            ...
            if ($bHas_thumb != FALSE)
            {
                ...
                $meta_query = array(
                    'key' => '_thumbnail_id',
                    'compare' => 'EXISTS');
                $meta_query = apply_filters('sfly_tbgrd_meta_query', $meta_query);
           ...
           if ($category__not_in)
            ...
            $settings->debug = $this->boolVal($debug_query);
